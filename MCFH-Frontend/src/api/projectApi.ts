...
export const projectApi = {
  getProjects: async (workspaceId: number): Promise<Project[]> => {
    const response = await axiosClient.get<unknown[]>(`/api/workspaces/${workspaceId}/projects`);
  ...
  update: async (
    workspaceId: number,
    projectId: number,
    payload: { name: string; description?: string | null; searchQuery?: string | null }
  ): Promise<Project> => {
...
export const scrapeApi = {
  startByKeyword: async (projectId: number): Promise<ScrapeJobStart> => {
  ...
  byKeyword: async (
    ...
    const { jobId } = await scrapeApi.startByKeyword(projectId);
    onProgress?.({
      jobId,
      projectId,
      status: 'running',
