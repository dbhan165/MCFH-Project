...
const Workspaces = () => {
  ...
  const loadWorkspaces = useCallback(async () => {
    ...
    try {
      ...
      setWorkspaces(data);
      const invites = await meApi.getInvitations();
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-16 space-y-8">
      ...
      {pendingInviteCount > 0 && (
        <Link
          to="/invitations"
          className="block rounded-2xl border border-[#FF7575]/30 bg-[#FF7575]/10 px-6 py-4 text-sm text-[#ffb4b4] hover:bg-[#FF7575]/15 transition-colors"
        >
          Bạn có <strong className="text-white">{pendingInviteCount}</strong> lời mời tham gia tổ chức — bấm để xem và phản hồi.
        </Link>
      )}
