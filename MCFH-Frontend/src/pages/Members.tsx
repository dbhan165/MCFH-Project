...
const Members = () => {
  ...
  const handleApprove = async (invitationId: number) => {
    ...
    try {
      ...
      setSuccessMessage('Đã duyệt lời mời.');
      await loadData();
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
              ...
              {isOwner && invitations.length > 0 && (
                <div className="mb-6 bg-[#151B2B] border border-yellow-500/20 rounded-2xl p-6">
                  <h3 className="font-bold mb-4 text-yellow-400 flex items-center gap-2">
                    <Clock size={18} /> Lời mời chờ duyệt
                  </h3>
                    ...
                    {invitations.map((inv) => (
                      <div key={inv.invitationId} className="flex items-center justify-between gap-4 p-4 bg-[#0A101D] rounded-xl border border-white/5">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{formatWorkspaceDateTime(inv.createdAt)}</p>
                        </div>
                        <button
                          onClick={() => handleApprove(inv.invitationId)}
                          className="px-4 py-2 rounded-lg bg-emerald-500/10 text-emerald-400 text-sm font-bold"
                        >
                          Duyệt
                        </button>
                      </div>
          ...
          <div className="bg-[#0A101D] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleInvite} className="p-6 space-y-5">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">Người được mời cần đã có tài khoản MCFH. Owner sẽ duyệt lời mời.</p>
              </div>
