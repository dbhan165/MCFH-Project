...
import { Filter, Loader2, AlertCircle, MessageSquare } from 'lucide-react';
...
import { loadReporterSettings } from './ReporterSettings';
...
const BespokeRequests = () => {
  ...
  const settings = loadReporterSettings();
  ...
  const cardPadding = settings.compactCards ? 'p-4' : 'p-5';
  const cardGap = settings.compactCards ? 'space-y-3' : 'space-y-4';
    ...
    <ReporterLayout activeTopNav="reports">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <h2 className="text-xl lg:text-2xl font-bold text-[#111827]">
          ...
          <span className="text-base font-normal text-[#78716c]">(Bespoke)</span>
        </h2>
        <button
          type="button"
          onClick={load}
          className="flex items-center gap-2 px-4 py-2.5 bg-white border border-stone-200 rounded-lg text-sm font-medium text-[#111827] hover:bg-stone-50 transition-colors shrink-0"
        >
            ...
            return (
              <div key={column.key} className="flex flex-col min-h-[320px]">
                <div className="flex items-center gap-2.5 mb-4 px-1">
                  ...
                  <span className="text-xs font-semibold text-[#78716c] bg-stone-100 px-2 py-0.5 rounded-full">
                    {cards.length}
                  </span>
                </div>

                <div className={`${cardGap} flex-1`}>
                  {cards.length === 0 ? (
                    ...
                    cards.map((card) => {
                      ...
                      const showNote =
                        settings.notifyClientNote && Boolean(card.revisionFeedback?.trim());
                      return (
                        <div
                          key={card.requestId}
                          className={`bg-white border border-stone-200 rounded-xl ${cardPadding} shadow-sm hover:shadow-md transition-shadow`}
                        >
                          ...
                          <div className="flex items-center gap-2.5 mb-3">
                            <span className="w-8 h-8 bg-rose-100 text-rose-700 rounded-md flex items-center justify-center text-xs font-bold shrink-0">
                              {initials}
                            </span>
                            <span className="text-sm text-[#78716c]">{card.clientName ?? '—'}</span>
                          </div>

                          {showNote && (
                            <p className="flex items-start gap-1.5 text-xs text-[#e11d48] bg-rose-50 border border-rose-100 rounded-lg px-2.5 py-2 mb-3 line-clamp-2">
                              <MessageSquare className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                              {card.revisionFeedback}
                            </p>
                          )}
                          ...
                          <button
                            ...
                            className="w-full py-2.5 border-2 border-[#e11d48] text-[#e11d48] hover:bg-rose-50 rounded-lg text-sm font-semibold transition-colors"
                          >
                            {label}
                          </button>
                        </div>
