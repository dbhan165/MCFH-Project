...
public class ProjectAnalyticsService
{
    ...
    public async Task<ProjectOverviewDto?> GetOverviewByProjectIdAsync(int projectId)
    {
        ...
        var feedbacks = await ProjectFeedbackQueryHelper.ForProject(_context, projectId)
    ...
    private async Task<IQueryable<ScrapedFeedback>> ProjectFeedbacksQueryAsync(int projectId)
    {
        await ProjectFeedbackQueryHelper.BackfillProjectIdsAsync(_context, projectId);
        return ProjectFeedbackQueryHelper.ForProject(_context, projectId);
    }
    ...
    public async Task<bool> DeleteMentionAsync(int workspaceId, int projectId, int userId, int feedbackId)
    {
        ...
        var feedback = await (await ProjectFeedbacksQueryAsync(projectId))
            .Include(f => f.AiAnalysis)
    ...
    public async Task<SentimentSummaryDto?> GetSentimentSummaryAsync(int workspaceId, int projectId, int userId)
    {
        ...
        var feedbacks = await (await ProjectFeedbacksQueryAsync(projectId))
            .Include(f => f.AiAnalysis)
            .ToListAsync();
