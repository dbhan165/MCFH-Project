...
public class TikTokScraper
{
    ...
    private static async Task FillFromDomAsync(IPage page, ScrapeResult result)
    {
        ...
        if (!result.PostedAt.HasValue)
        {
            var dateText = await TryGetTextAsync(page,
                "[data-e2e='browser-nickname'] + span",
                "[data-e2e='video-creation-time']",
                "span[data-e2e='video-creation-time']",
                "span[class*='SpanCreateTime']",
        ...
        try
        {
            var raw = await page.EvaluateAsync<string?>(@"() => {
                const metas = [
                    ...
                    document.querySelector('meta[name=""created""]')?.getAttribute('content')
                ];
                return metas.find(Boolean) || null;
            }");

            if (PostedAtParser.TryParseAny(raw, out var fromMeta))
                result.PostedAt = fromMeta;
        }
        catch { }
    }
    ...
    private static async Task FillFromEmbeddedJsonAsync(IPage page, ScrapeResult result)
    {
        try
        {
            ...
            TryExtractCreateTimeFromPaths(doc.RootElement, result);
            WalkJson(doc.RootElement, result);
        }
        catch { }
    }

    private static void TryExtractCreateTimeFromPaths(JsonElement root, ScrapeResult result)
    {
        if (result.PostedAt.HasValue) return;

        string[][] paths =
        [
            ["webapp.video-detail", "itemInfo", "itemStruct", "createTime"],
            ["webapp.video-detail", "itemInfo", "itemStruct", "create_time"],
            ["ItemModule"],
        ];

        foreach (var path in paths)
        {
            if (!TryNavigate(root, path, out var node)) continue;

            if (path[^1] == "ItemModule" && node.ValueKind == JsonValueKind.Object)
            {
                foreach (var item in node.EnumerateObject())
                {
                    if (item.Value.TryGetProperty("createTime", out var ct) &&
                        PostedAtParser.TryParseUnixToken(ct, out var fromItem))
                    {
                        result.PostedAt = fromItem;
                        return;
                    }
                }
                continue;
            ...
            if (PostedAtParser.TryParseUnixToken(node, out var fromPath))
            {
                result.PostedAt = fromPath;
                return;
            }
        }
    }

    private static bool TryNavigate(JsonElement root, string[] path, out JsonElement node)
    {
        node = root;
    ...
    private static void WalkJson(JsonElement element, ScrapeResult result)
    {
        switch (element.ValueKind)
        {
            case JsonValueKind.Object:
                ...
                if (!result.PostedAt.HasValue && element.TryGetProperty("createTime", out var createTime))
                {
                    if (PostedAtParser.TryParseUnixToken(createTime, out var fromCreate))
                        result.PostedAt = fromCreate;
                }

                if (!result.PostedAt.HasValue && element.TryGetProperty("create_time", out var createTimeSnake))
                {
                    if (PostedAtParser.TryParseUnixToken(createTimeSnake, out var fromSnake))
                        result.PostedAt = fromSnake;
                }
